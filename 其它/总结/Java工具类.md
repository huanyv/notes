...
```java
BufferedImage image1 = ImageIO.read(new File("C:\\Users\\wjb_0\\Desktop\\a.jpg"));
BufferedImage image2 = ImageIO.read(new File("C:\\Users\\wjb_0\\Desktop\\b.jpg"));
AnimatedGifEncoder e = new AnimatedGifEncoder();

// 设置生成图片大小
e.setSize(100, 100);
// 生成的图片路径
e.start("C:\\Users\\wjb_0\\Desktop\\c.gif");
...
public class Main implements Runnable {
    ...
    @Option(names = {"-w", "-width"}, description = "输出图片宽度")
    private int width;

    @Option(names = {"-h", "-height"}, description = "输出图片高度")
    private int height;

    @Option(names = {"-d", "-delay"}, defaultValue = "1000", description = "图片时间间隔, 默认1s")
    private int delay;

    @Option(names = {"-r", "-repeat"}, defaultValue = "true", description = "是否无限循环, 默认是")
    private boolean repeat;

    @Option(names = {"-o", "-output"}, defaultValue = "output.gif", description = "输出文件名, 默认output.gif")
    private String output;

    @Parameters(paramLabel = "<files>", description = "文件列表, 至少两张图片")
    private String[] files;

    public static void main(String[] args) throws Throwable {
        new CommandLine(new Main()).execute(args);
        // new CommandLine(new Main()).execute("a.jpg", "b.jpg");
    }

    @Override
    public void run() {
        if (files.length < 2) {
            System.out.println("请选择多张图片");
            return;
        }
        // ....................
